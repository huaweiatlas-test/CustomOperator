...
Status Caffecustom_ExpParseParams(const Message* op_origin, ge::Operator& op_dest)
{
...
Status Caffecustom_ExpInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
...
Status Caffecustom_ExpBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FuncName   = "custom_Exp";
    KernelName = "custom_Exp_" + std::to_string(input1_desc.GetShape().GetDim(0)) + "_" +  std::to_string(input1_desc.GetShape().GetDim(1)) + "_" +
...
REGISTER_CUSTOM_OP("custom_exp_param") //test_custom_Exp is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. It is case sensitive. 
    ...
    .ParseParamsFn(Caffecustom_ExpParseParams)  // Op parameters parse function
    .InferShapeAndTypeFn(Caffecustom_ExpInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(Caffecustom_ExpBuildTeBin)           // Build Te op binary function
