...
Status Caffecustom_LogParseParams(const Message* op_origin, ge::Operator& op_dest)
{
    ...
    const caffe::custom_LogParameter& param = layer->custom_log_param();
	...
	op_dest.SetAttr("base", AttrValue::CreateFrom<AttrValue::FLOAT>(param.base()));  
    
    return SUCCESS;
}

// #### Obtains the processing function of the output tensor description. 
Status Caffecustom_LogInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
...
Status Caffecustom_LogBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FilePath   = "../operator/custom_Log";
    FuncName   = "custom_Log";
    KernelName = "custom_Log_" + std::to_string(input1_desc.GetShape().GetDim(0)) + "_" +  std::to_string(input1_desc.GetShape().GetDim(1)) + "_" +
...
REGISTER_CUSTOM_OP("custom_log_param") //test_custom_Log is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. It is case sensitive. 
    .FrameworkType(CAFFE)  // Enumerated type. The options are as follows: CAFFE, TENSORFLOW
    .OriginOpType("custom_Log")  // custom_Log indicates the type name of the operator in the caffe framework.
    .ParseParamsFn(Caffecustom_LogParseParams)  // Op parameters parse function
    .InferShapeAndTypeFn(Caffecustom_LogInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(Caffecustom_LogBuildTeBin)           // Build Te op binary function
