...
@util.check_input_type((list, tuple), str, (int, float), (int, float), (int, float), str, bool, bool)
def custom_Log(shape, dtype, gamma=-1, alpha=1, beta=0, kernel_name="cce_caffe_log_layer",
                        need_build=False, need_print=False):
    """
    calculate y = log(gamma, alphax+beta) = log(alphax+beta) / log(gamma) for gamma > 0, calculating data type is float16 or float32
    ...
    if gamma != -1 and gamma <= 0:  # api  cc_device_exp_c handle gamma == -1 as e
        raise ValueError("please ensure gamma is greater than 0, where gamma = %s" % str(gamma))
    ...
    output = tvm.extern(shape, [data_input, p_scale, p_shift, p_base, p_shape], lambda ins, outs:
