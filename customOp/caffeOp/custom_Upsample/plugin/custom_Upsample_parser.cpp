...
Status Caffecustom_UpsampleParseParams(const Message* op_origin, ge::Operator& op_dest)
{
	...
	{
		op_dest.SetAttr("data_format", AttrValue::CreateFrom<AttrValue::STR>(param.data_format()));  
	}
	if(param.has_scale()) 
	{
...
Status Caffecustom_UpsampleInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
    auto tensorDesc      = op.GetInputDesc(0);
    auto shape = tensorDesc.GetShape();
	string data_format = "channels_first";
	ge::AttrValue data_formatAttrValue;
	if ((ge::GRAPH_SUCCESS != op.GetAttr("data_format", data_formatAttrValue))
		|| (ge::GRAPH_SUCCESS != data_formatAttrValue.GetValue<ge::AttrValue::STR>(data_format)))
	{
...
Status Caffecustom_UpsampleBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FilePath   = "../operator/custom_Upsample";
    FuncName   = "custom_Upsample";
    KernelName = "custom_Upsample_" + std::to_string(op.GetInputDesc(0).GetShape().GetDim(0)) + "_" +  std::to_string(op.GetInputDesc(0).GetShape().GetDim(1)) + "_" +
...
REGISTER_CUSTOM_OP("custom_upsample_param") //test_custom_Upsample is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. It is case sensitive. 
    ...
    .ParseParamsFn(Caffecustom_UpsampleParseParams)  // Op parameters parse function
    .InferShapeAndTypeFn(Caffecustom_UpsampleInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(Caffecustom_UpsampleBuildTeBin)           // Build Te op binary function
