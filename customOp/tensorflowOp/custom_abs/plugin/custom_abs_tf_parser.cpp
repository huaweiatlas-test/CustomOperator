...
Status TFcustom_absInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
...
Status TFcustom_absBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FilePath   = "../operator/custom_abs";
    FuncName   = "custom_abs";
    KernelName = "custom_abs" + std::to_string(input_desc.GetShape().GetDim(0)) + std::to_string(input_desc.GetShape().GetDim(1));
...
REGISTER_CUSTOM_OP("custom_abs") //custom_abs is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. It is case sensitive. 
    ...
    .InferShapeAndTypeFn(TFcustom_absInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(TFcustom_absBuildTeBin) // Build Te op binary function
