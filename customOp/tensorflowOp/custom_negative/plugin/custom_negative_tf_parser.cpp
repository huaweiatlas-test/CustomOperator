...
Status TFcustom_negativeInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
...
Status TFcustom_negativeBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FilePath   = "../operator/custom_negative";
    FuncName   = "custom_negative";
    KernelName = "custom_negative" + std::to_string(input_desc.GetShape().GetDim(0)) + std::to_string(input_desc.GetShape().GetDim(1));
...
REGISTER_CUSTOM_OP("custom_negative") // custom_negative is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. 
    .FrameworkType(TENSORFLOW)  // Enumerated type. The options are as follows: CAFFE, TENSORFLOW
    .OriginOpType("Neg")  // Neg indicates the type name of the operator in the caffe framework.
    .ParseParamsFn(AutoMappingFn)  // AutoMappingFn indicates automatic mapping the parameters of op.
    .InferShapeAndTypeFn(TFcustom_negativeInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(TFcustom_negativeBuildTeBin) // Build Te op binary function
