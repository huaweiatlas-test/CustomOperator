...
Status TFcustom_cumprodInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
	...
	ge::AttrValue exclusiveAttrValue;  
	if ((ge::GRAPH_SUCCESS != op.GetAttr("exclusive", exclusiveAttrValue)) || (ge::GRAPH_SUCCESS != exclusiveAttrValue.GetValue<AttrValue::BOOL>(exclusive)))  
	{  
	    printf("Get exclusive failed!\n ");  
	}  
	bool reverse = false;  
	ge::AttrValue reverseAttrValue;  
	if ((ge::GRAPH_SUCCESS != op.GetAttr("reverse", reverseAttrValue)) || (ge::GRAPH_SUCCESS != reverseAttrValue.GetValue<AttrValue::BOOL>(reverse)))  
	{  
	    printf("Get reverse failed!\n ");  
	}  
...
Status TFcustom_cumprodBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FilePath   = "../operator/custom_cumprod";
    FuncName   = "custom_cumprod";
    KernelName = "custom_cumprod" + std::to_string(input_desc.GetShape().GetDim(0)) + std::to_string(input_desc.GetShape().GetDim(1));
...
REGISTER_CUSTOM_OP("custom_cumprod") //custom_cumprod is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. 
    .FrameworkType(TENSORFLOW)  // Enumerated type. The options are as follows: CAFFE, TENSORFLOW
    .OriginOpType("Cumprod")  // // Cumprod indicates the type name of the operator in the caffe framework.
    .ParseParamsFn(AutoMappingFn)  // AutoMappingFn indicates automatic mapping the parameters of op.
    .InferShapeAndTypeFn(TFcustom_cumprodInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(TFcustom_cumprodBuildTeBin) // Build Te op binary function
