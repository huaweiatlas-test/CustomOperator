...
Status TFcustom_tileInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
    ...
    shape.SetDim(0, shape.GetDim(0)*shape_2.GetDim(0));
    shape.SetDim(1, shape.GetDim(1)*shape_2.GetDim(1));
    shape.SetDim(2, shape.GetDim(2)*shape_2.GetDim(2));
    shape.SetDim(3, shape.GetDim(3)*shape_2.GetDim(3));
...
Status TFcustom_tileBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    KernelName = "custom_tile" + std::to_string(input_desc.GetShape().GetDim(0)) + std::to_string(input_desc.GetShape().GetDim(1));
    ...
    te::BuildTeCustomOp(te_bin_info.ddk_version, op.GetName(), RealFilePath, FuncName,
		...
		input_desc_2.GetShape().GetDim(0),
		dtype.c_str(),
		KernelName.c_str(),
        Py_True);
...
REGISTER_CUSTOM_OP("custom_tile") // custom_tile is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. It is case sensitive. 
    .FrameworkType(TENSORFLOW)  // Enumerated type. The options are as follows: CAFFE, TENSORFLOW
    .OriginOpType("Tile")  // Tile indicates the type name of the operator in the caffe framework.
    .ParseParamsFn(AutoMappingFn)  // AutoMappingFn indicates automatic mapping the parameters of op.
    .InferShapeAndTypeFn(TFcustom_tileInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(TFcustom_tileBuildTeBin) // Build Te op binary function
