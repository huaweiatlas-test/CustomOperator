...
def custom_truncatemod(shape1, shape2, dtype, kernel_name="cce_tf_truncatemod", need_build=False, need_print=False):
    """
    ...
    if shape1_len > max_dim or shape2_len > max_dim:
        raise RuntimeError("mod_cce only support up to %d dimensions while the shape's dimensions is %d, %d" %
    ...
    if not (dtype in check_list):
        raise RuntimeError("tf_truncatemod_cce only support %s while dtype is %s" % (",".join(check_list), dtype))
    ...
    output = tvm.extern(shape_out, [p_xshape, data_input_x, p_yshape, data_input_y, p_out_shape], lambda ins, outs:
