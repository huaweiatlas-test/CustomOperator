...
Status TFcustom_logInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
...
Status TFcustom_logBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FuncName   = "custom_log";
    KernelName = "custom_log" + std::to_string(input_desc.GetShape().GetDim(0)) + std::to_string(input_desc.GetShape().GetDim(1));
...
REGISTER_CUSTOM_OP("custom_log") // custom_log is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. It is case sensitive. 
    .FrameworkType(TENSORFLOW)  // Enumerated type. The options are as follows: CAFFE, TENSORFLOW
    .OriginOpType("Log")  // Log indicates the type name of the operator in the caffe framework.
    .ParseParamsFn(AutoMappingFn)  // AutoMappingFn indicates automatic mapping the parameters of op.
    .InferShapeAndTypeFn(TFcustom_logInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(TFcustom_logBuildTeBin) // Build Te op binary function
