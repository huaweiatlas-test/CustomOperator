...
Status TFcustom_greater_equalInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
...
Status TFcustom_greater_equalBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FilePath   = "../operator/custom_greater_equal";
    FuncName   = "custom_greater_equal";
    KernelName = "custom_greater_equal" + std::to_string(input_desc.GetShape().GetDim(0)) + std::to_string(input_desc.GetShape().GetDim(1));
...
REGISTER_CUSTOM_OP("custom_greater_equal") //custom_greater_equal is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name.
    ...
    .InferShapeAndTypeFn(TFcustom_greater_equalInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(TFcustom_greater_equalBuildTeBin) // Build Te op binary function
