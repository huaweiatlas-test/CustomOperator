...
Status TFcustom_squared_differenceInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
...
Status TFcustom_squared_differenceBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FilePath   = "../operator/custom_squared_difference";
    FuncName   = "custom_squared_difference";
    KernelName = "custom_squared_difference" + std::to_string(input_desc.GetShape().GetDim(0)) + std::to_string(input_desc.GetShape().GetDim(1));
...
REGISTER_CUSTOM_OP("custom_squared_difference") // custom_squared_difference is the type name of the operator in the OM model. It can be specified randomly and it is case sensitive. 
    .FrameworkType(TENSORFLOW)  // Enumerated type. The options are as follows: CAFFE, TENSORFLOW
    .OriginOpType("SquaredDifference")  // SquaredDifference indicates the type name of the operator in the caffe framework.
    .ParseParamsFn(AutoMappingFn)  // AutoMappingFn indicates automatic mapping the parameters of op.
    .InferShapeAndTypeFn(TFcustom_squared_differenceInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(TFcustom_squared_differenceBuildTeBin) // Build Te op binary function
