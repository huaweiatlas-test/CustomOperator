...
def custom_ceil(shape, dtype, kernel_name="cce_tf_ceil", need_build=False, need_print=False):
    """
    ...
    if shape_len > max_dim:
        raise RuntimeError(
            "ceil_cce only support up to %d dimensions while the shape's dimension is %d" %
            (max_dim, shape_len))
    ...
    if not (dtype.lower() in check_list):
        raise RuntimeError(
            "ceil_cce only support %s while dtype is %s" % (",".join(check_list), dtype))
