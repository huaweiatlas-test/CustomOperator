...
def custom_expm1(shape, dtype, kernel_name="cce_tf_expm1", need_build=False, need_print=False):
    """
    algorithm: expm1
    ...
    if not (dtype.lower() in supported_dtypes):
        raise RuntimeError("tf_expm1_cce only support %s while dtype is %s" % (",".join(supported_dtypes), dtype))
    ...
    output_exp = tvm.extern(shape, [data_input, p_scale, p_shift, p_base, p_shape], lambda ins, outs:
    ...
    output = tvm.compute(shape, lambda *indice: output_exp(*indice) + offset.astype(inp_dtype), name="output")
