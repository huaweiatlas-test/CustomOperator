...
SHAPE_SIZE_LIMIT =  100000000  # shape limit for tf_logical_and

@util.check_input_type((list, tuple), (list, tuple), str, str, bool, bool)
def custom_logical_and(shape_x, shape_y, dtype, kernel_name="cce_tf_logical_and", need_build=False,
                       need_print=False):
    """
    ...
    if not (dtype.lower() in check_list):
        raise RuntimeError(
            "logical_and_cce only support %s while dtype is %s" % (",".join(check_list), dtype))
    ...
    with tvm.target.cce():
        ...
        res = tvm.compute(shape_max, lambda *i: tvm.select(
            tvm.all(tvm.any(data1_tmp1(*i) > min_value, data1_tmp1(*i) < -min_value),
                    tvm.any(data1_tmp2(*i) > min_value, data1_tmp2(*i) < -min_value)), True, False),
                          name="res")
