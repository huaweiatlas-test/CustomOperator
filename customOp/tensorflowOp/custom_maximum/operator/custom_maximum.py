...
def custom_maximum(shape1, shape2, dtype, kernel_name="cce_tf_maximum", need_build=False,
                   need_print=False):
    """
    ...
    if not (dtype in check_list):
        raise RuntimeError(
            "tf_maximum_cce only support %s while dtype is %s" % (",".join(check_list), dtype))
