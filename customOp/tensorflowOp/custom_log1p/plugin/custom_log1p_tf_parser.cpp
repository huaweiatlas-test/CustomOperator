...
Status TFcustom_log1pInferShapeAndType(const ge::Operator& op, vector<ge::TensorDesc>& v_output_desc)
{
...
Status TFcustom_log1pBuildTeBin(const ge::Operator& op, TEBinInfo& te_bin_info)
{
    std::string FilePath   = "";
    ...
    FilePath   = "../operator/custom_log1p";
    FuncName   = "custom_log1p";
    KernelName = "custom_log1p" + std::to_string(input_desc.GetShape().GetDim(0)) + std::to_string(input_desc.GetShape().GetDim(1));
...
REGISTER_CUSTOM_OP("custom_log1p") // custom_log1p is the type name of the operator in the OM model. It can be specified randomly and cannot be the same as an existing type name. It is case sensitive. 
    ...
    .InferShapeAndTypeFn(TFcustom_log1pInferShapeAndType)       // Set output description and datatype function
    .TEBinBuildFn(TFcustom_log1pBuildTeBin) // Build Te op binary function
