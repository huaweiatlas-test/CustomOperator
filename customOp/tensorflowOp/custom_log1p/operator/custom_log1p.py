...
def custom_log1p(shape, dtype, kernel_name="cce_tf_log1p", need_build=False, need_print=False):
    """
    ...
    if not (dtype.lower() in supported_dtypes):
        raise RuntimeError("tf_log1p_cce only support %s while dtype is %s" % (",".join(supported_dtypes), dtype))
