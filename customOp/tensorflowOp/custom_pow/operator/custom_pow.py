...
def custom_pow(shape, shape_y, dtype, kernel_name="cce_tf_pow", need_build=False, need_print=False):
    """
    ...
    if not (dtype.lower() in supported_dtypes):
        raise RuntimeError(
            "tf_pow_cce only support %s while dtype is %s" % (",".join(supported_dtypes), dtype))
    ...
    output = tvm.extern(shape, [data_lhs, data_rhs, p_scale, p_shift, p_power, p_shape],
                        lambda ins, outs:
    ...
    if need_print:
        with build_config:
            print(tvm.lower(s, [data_lhs, data_rhs, output], simple_mode=True))
    if need_build:
        with build_config:
            tvm.build(s, [data_lhs, data_rhs, output], "cce", name=kernel_name)
